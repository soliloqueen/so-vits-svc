...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, audiopaths, hparams):
        ...
        self.spk_filemap = {}
        self.spk_pdf = {}
        for fn in self.audiopaths:
            fn = fn[0]
            spk = fn.replace('\\','/').split('/')[-2]
            if not spk in self.spk_filemap:
                self.spk_filemap[spk] = []
            self.spk_filemap[spk].append(fn)
        for k,v in self.spk_filemap.items():
            self.spk_pdf[k] = np.tanh(len(v)/len(self.audiopaths) + 0.2)
        pd_sum = sum(self.spk_pdf.values())
        for k,v in self.spk_filemap.items():
            self.spk_pdf[k] = self.spk_pdf[k]/pd_sum
        print(self.spk_pdf)
    ...
    def __getitem__(self, index):
        spk = np.random.choice(
            list(self.spk_filemap.keys()), size=1, p=list(self.spk_pdf.values()))[0]
        pth = np.random.choice(self.spk_filemap[spk], size=1)[0]
        c, f0, spec, audio_norm, spk = self.get_audio(pth)
        return c, f0, spec, audio_norm, spk
